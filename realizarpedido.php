<?php
   ...
   if(!trim($_POST['billing_first_name'])){
	   header('Location: http://urglo.com/miembros/pedido.php?error=El nombre es requerido');
	   die;
   }
   if(!filter_var($_POST['billing_email'], FILTER_VALIDATE_EMAIL)){
	   header('Location: http://urglo.com/miembros/pedido.php?error=El correo no es valido');
	   die;
   }
   ...
   if(strpos($imgfile["type"], "image") !== FALSE) {
	 ...
	 header('Location: http://urglo.com/miembros/pedido.php?error=El archivo que has subido no es una imagen valida');
   }
   ...
   if(!$l){ header('Location: http://urglo.com/miembros/pedido.php?error=Hubo un error subiendo el archivo a '.$dcm.'/'.$urlnew); die; }
   //$urlimg = $_SERVER['HTTP_HOST'].'/'.$urlnew;
   $urlimg = 'http://urglo.com/miembros/'.$urlnew;
   ...
   mail('ventas@urglo.com', 'Recibo de compra', $htmlm, $cabeceras);
   header('Location: http://urglo.com/miembros/pedido.php?success=El recibo ha sido enviado correctamente');
?>