...
class Usuarios extends Conexion {
	...
	{
		if($_POST['nombre'] == '' || $_POST['apellido'] == '' || $_POST['email'] == '' || $_POST['telefono'] == '' || $_POST['pais'] == '' || $_POST['user_lolcoach'] == '' || $_POST['pass_lolcoach'] == '' || $_POST['pass2_lolcoach'] == '')
		{
			echo 'Por favor, complete todos los campos.';
		}
		else
		{
			...
			$pais = $_POST['pais'];
			$user_lolcoach = $_POST['user_lolcoach'];
			$pass_lolcoach = $_POST['pass_lolcoach'];
			$pass2_lolcoach = $_POST['pass2_lolcoach'];
			...
			if(check_email($email))
			{
				...
				{
					if($this->comprobar_nick($user_lolcoach) == true)
					{
						echo 'El nombre de usuario en lolcoach ya esta en uso.';
					}
					else
					{
						if($pass_lolcoach == $pass2_lolcoach)
						{
							$this->conectar_db();
							$this->codigosql = "START TRANSACTION";
 							...
 							if($this->consulta)
 							{
 								$pass_lolcoach = md5($pass_lolcoach);
			 					...
			 					$consulta2 = mysqli_query($this->conexion, "INSERT INTO usuarios VALUES (null, '$user_lolcoach', '$pass_lolcoach', 'user')");
								...
								if($consulta1 && $consulta2)
								{
									...
									$this->consulta = mysqli_query($this->conexion, $this->codigosql);
									$this->desconectar_db();
									$this->conectar_usuario($user_lolcoach, $pass_lolcoach);
									return true;
								}
								else
								{
			...
			else
			{
				...
				else
				{
					...
					else
					{
					    ...
					    $filepath = $_SERVER['DOCUMENT_ROOT'] . '/miembros/images/user/';
			...
			if($this->consulta)
			{
				...
				{		
					...
					if($consulta6 && $consulta7)
					{
						...
						echo 'Felicitaciones ya esta todo listo. Asi quedo tu perfil <a href="http://urglo.com/miembros/coachs/'.$usuario.'">Mi Perfil de Coach</a>';
					}
					else
					{
						$this->codigosql = 'rollback;';
