				...
				if(isset($url[1]))
				{
					switch ($url[1]) 
					{
									...
									{	
										...
										$lista_mensajes.='
											<div class="message-user">
								            	<img src="http://urglo.com/miembros/images/user/'.$datos_personal_envia[0]['img_perfil'].'" width="32px" height="32px">
						...
						case ''.$url[1].'':
							...
							if($resultado == false)
							{
								header('Location: http://'.$_SERVER['HTTP_HOST'].'/miembros/mensajes');
							}
							else
							{
		...
		switch ($url[0]) 
		{
			case 'mensajes':
				if(isset($url[1]))
				{
					...
					{
						case '':
							require_once('vistas/coacher/logicas/mensajes.php');
						break;
						...
						case 'bandeja_salida':
							require_once('vistas/coacher/logicas/mensajes_bandeja_salida.php');
						break;

						case 'enviar':
							require_once('vistas/coacher/logicas/enviar_mensaje.php');
						break;

						case 'enviar_mensaje_accion':
							require_once('vistas/coacher/logicas/enviar_mensaje_accion.php');
						break;

						case 'responder_mensaje_accion':
							require_once('vistas/coacher/logicas/responder_mensaje_accion.php');
						break;
						...
						case ''.$url[1].'':
							...
							{
								require_once('vistas/coacher/logicas/ver_mensaje.php');
							}
						break;
					}
				}
				...
				if(isset($url[1]))
				{
					switch ($url[1]) 
					{
						...
						case 'conversacion_coaching':
							...
							if($chat == false)
							{
								echo '<center>No hay ningún mensaje entre el usuario '.$user[0]['usuario'].' y el coacher '.$elo[0]['usuario'].'</center>';
							}
							else
							{ 
								$mensajes_chat = $mensajes->mensajes_chat_coaching($chat[0]['id_conversacion']);
