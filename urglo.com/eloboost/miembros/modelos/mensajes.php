...
class Mensajes extends Conexion {
	...
	public function mandar_email_eloboosters()
	{
		$this->codigosql = "select * FROM usuarios,usuario_personal WHERE tipo='elobooster' and usuario_personal.id_usuario = usuarios.id_usuario";		
		$this->obtener_resultados();
		if($this->num_resultados > 0)
		{
			return $this->resultado;
	...
	public function enviar_mensaje($id_remitente, $id_receptor, $asunto, $contenido)
	{
		...
		{
			echo 'Rellena todos los campos para enviar el ticket.';
		}
		else
		{
			...
			else
			{
				...
				{
					echo 'El ticket de soporte ya habia sido enviado.';
				}
				else
				{
					$hora = date("H:i:s");
					...
					{
						echo 'Hubo un error en enviar el ticket, por favor intentelo denuevo.';
					}
				}
			}	
		}
	...
	/////////////////////////////// CHAT BOOSTING ////////////////////////////////////
	public function conversacion_boosting($id_trabajo)
	{
		$this->codigosql = "SELECT * FROM conversacion_boosting WHERE id_pedido='$id_trabajo'";
		$this->obtener_resultados();
	...
	public function mensajes_chat_boosting($id_conversacion)
	{
		$this->codigosql = "SELECT * FROM chat_boosting WHERE id_conversacion='$id_conversacion' order by id_mensaje asc";
		$this->obtener_resultados();
	...
	public function enviar_msj_chat_boosting($id_conversacion, $id_trabajo, $mensaje, $id_envia, $id_receptor)
	{
		//Con esto pregunto si traigo mensajes, si no trae es porque no hay mensajes, tampoco conversacion
		$this->codigosql = "SELECT id_mensaje FROM chat_boosting WHERE id_conversacion='$id_conversacion' order by id_mensaje desc LIMIT 1";
		$this->obtener_resultados();
		if($this->num_resultados > 0) 
		{
			//si ya existe la conver va sumando el id del mensaje por cada mensaje que envia
			...
			$hora = date("H:i:s");
			$fecha = date("Y-m-d");
			$this->codigosql = "INSERT INTO chat_boosting (id_conversacion, id_mensaje, mensaje, id_user_envia, fecha_envio, hora_envio) VALUES ('$id_conversacion', '$id_mensaje'+1, '$mensaje', '$id_envia', '$fecha', '$hora')";
			$this->consulta_simple();
			if($this->consulta)
			{
				return true;
		...
		{
			// si no existe entra aca y busca el ultimo id conversacion
			$this->codigosql = "SELECT id_conversacion FROM chat_boosting order by id_conversacion desc LIMIT 1";
			$this->obtener_resultados();
			if($this->num_resultados > 0) 
			{
				//si entra aca es porque trajo el ultimo  id conversacion para sumarlo entonces crea nueva conver
				...
				$hora = date("H:i:s");
				$fecha = date("Y-m-d");
				$this->codigosql = "INSERT INTO conversacion_boosting VALUES ('$id_conversacion'+1, '$id_trabajo', '$fecha', '$hora')";
				$this->consulta_simple();
				if($this->consulta)
				{
					$hora = date("H:i:s");
					$fecha = date("Y-m-d");
					$this->codigosql = "INSERT INTO chat_boosting (id_conversacion, id_mensaje, mensaje, id_user_envia, fecha_envio, hora_envio) VALUES ('$id_conversacion'+1, '1', '$mensaje', '$id_envia', '$fecha', '$hora')";
					$this->consulta_simple();
					if($this->consulta)
					{
						return true;
			...
			else
			{
				...
				$hora = date("H:i:s");
				$fecha = date("Y-m-d");
				$this->codigosql = "INSERT INTO conversacion_boosting VALUES ('1', '$id_trabajo', '$fecha', '$hora')";
				$this->consulta_simple();
				if($this->consulta)
				{
					$hora = date("H:i:s");
					$fecha = date("Y-m-d");
					$this->codigosql = "INSERT INTO chat_boosting (id_conversacion, id_mensaje, mensaje, id_user_envia, fecha_envio, hora_envio) VALUES ('1', '1', '$mensaje', '$id_envia', '$fecha', '$hora')";
					$this->consulta_simple();
					if($this->consulta)
					{
						return true;
