...
class Usuarios extends Conexion {
	...
	{
		if($_POST['nombre'] == '' || $_POST['apellido'] == '' || $_POST['email'] == '' || $_POST['telefono'] == '' || $_POST['pais'] == '' || $_POST['user_lolboost'] == '' || $_POST['pass_lolboost'] == '' || $_POST['pass2_lolboost'] == '')
		{
			echo 'Por favor, complete todos los campos.';
		}
		else
		{
			...
			$pais = $_POST['pais'];
			$user_lolboost = $_POST['user_lolboost'];
			$pass_lolboost = $_POST['pass_lolboost'];
			$pass2_lolboost = $_POST['pass2_lolboost'];
			...
			if(check_email($email))
			{
				...
				{
					if($this->comprobar_nick($user_lolboost) == true)
					{
						echo 'El nombre de usuario en urglo.com/eloboost ya esta en uso.';
					}
					else
					{
						if($pass_lolboost == $pass2_lolboost)
						{
							$this->conectar_db();
							$this->codigosql = "START TRANSACTION";
 							...
 							if($this->consulta)
 							{
 								$pass_login1 = $pass_lolboost;
 								$pass_lolboost = md5($pass_lolboost);
 								$consulta1 = mysqli_query($this->conexion, "INSERT INTO usuario_personal VALUES (null, '$nombre', '$apellido', '$email', '$paypal', '$telefono', '$pais', 'user_perfil_defecto.jpg')");
			 					$consulta2 = mysqli_query($this->conexion, "INSERT INTO usuarios VALUES (null, '$user_lolboost', '$pass_lolboost', 'user')");
								...
								if($consulta1 && $consulta2)
								{
									...
									$this->consulta = mysqli_query($this->conexion, $this->codigosql);
									$this->desconectar_db();
									$this->conectar_usuario($user_lolboost, $pass_login1);
									return true;
								}
								else
								{
	...
	public function editar_perfil($id_usuario, $username, $password, $tipo_cuenta, $nombre, $apellido, $correo, $paypal, $telefono, $pais)
	{
		...
		{
			echo 'Por favor, complete todos los campos para poder realizar los cambios';
		}
		else
		{
	...
	public function editar_personal($id_usuario, $nombre, $apellido, $correo, $paypal, $telefono)
	{
		...
		else
		{
			...
			{
				echo 'Hubo un error al realizar los cambios, por favor intentelo denuevo.';
			}	
		}
	}
			...
			else
			{
				...
				else
				{
					...
					else
					{
					    ...
					    $filepath = $_SERVER['DOCUMENT_ROOT'] . '/eloboost/miembros/images/user/';
						...
						{
						    echo 'Hubo un error al subir tu imagen de perfil.';
						}
					}
				}
			}
		...
		else
		{
			if($this->comprobar_pass($id_usuario, $old) == true)
			{
				if($nueva == $rep)
				{
					...
					if($this->consulta)
					{
						echo 'La contraseña a sido cambiada correctamente.';
					}
					else
					{
						echo 'Hubo un error al cambiar la contraseña, por favor intentelo denuevo.';
					}	
				}
				else
				{
	...
	public function lista_usuarios_elooboster()
	{
		$this->codigosql = "SELECT * FROM usuarios WHERE tipo='eloobosters'";
		$this->obtener_resultados();
			...
			if($this->consulta)
			{
				...
				{		
					...
					if($consulta6 && $consulta7)
					{
						...
						echo 'Felicitaciones ya esta todo listo. Asi quedo tu perfil <a href="http://urglo.com/eloboost/miembros/coachs/'.$usuario.'">Mi Perfil de Coach</a>';
					}
					else
					{
						$this->codigosql = 'rollback;';
	...
	{
		$this->codigosql = "SELECT id_coach, sum(calificacion) FROM calificacion_coach GROUP BY id_coach";
		$this->obtener_resultados();
		if($this->num_resultados > 0)
		{
			return $this->resultado;
