<?php
	$html = file_get_contents('vistas/user/perfil_coach.html');
	...
	$calificaciones = $trabajos->calificaciones_coach($datos_personal_coach[0]['id_usuario']);
	$total = '';
	...
	for($i=0;$i<count($roles);$i++)
	{
		...
		$roles_coach.='<img src="http://urglo.com/eloboost/miembros/images/Roles/'.$rol[0]['img'].'" width="30" align="center" title="'.$rol[0]['nombre'].'">';
	}
	if($roles == false)
	{
	...
	for($i=0;$i<count($idiomas);$i++)
	{
		...
		$idiomas_coach.='<img src="http://urglo.com/eloboost/miembros/images/Languages/'.$idioma[0]['bandera'].'" width="30" align="center" title="'.$idioma[0]['idioma'].'">'; 
	}
	if($idioma == false)
	{
	...
	$lol_servers = $usuarios->lista_server_lol();
	$lol_servers_lista = '';
	for($i=0;$i<count($lol_servers);$i++)
	{
		$lol_servers_lista.= '<option value="'.$lol_servers[$i]['id_server'].'">'.$lol_servers[$i]['nombre'].'</option>';
	}
	$html = str_replace('{lol_servers_options}', $lol_servers_lista, $html);
		...
		for($i=0;$i<count($opiniones);$i++)
		{
			...
			<div class="pad10" style="background:#eee; margin-top:10px;">
                <div class="ratingSmall">
                    <img class="rat'.$opiniones[$i]['calificacion'].'" src="http://urglo.com/eloboost/miembros/images/rating-stars-small.png" align="middle">
                </div>
	...
	if($pais_usuario == 'Chile')
	{
		...
		<label>
			...
			<i>
			    ...
			    <input type="radio" value="kiphu" name="pays" style="box-shadow: none;"/>
			</i>
		</label>
		<label>
			<span style="width:130px;text-align:left">Paga en efectivo</span>
			<i>
			    <img src="http://urglo.com/eloboost/miembros/images/efectivo.png" width="40px" align="middle"/>
			    <input type="radio" value="efectivo" name="pays" style="box-shadow: none;"/>
			</i>
		</label>
		';
		$chi_t = '
		<label>
			...
			<i>
			    <img src="http://urglo.com/eloboost/miembros/images/kiphu.png" width="90px" align="middle"/>
			    <input type="radio" value="kiphu" name="payt" style="box-shadow: none;"/>
			</i>
		</label>
		<label>
			<span style="width:130px;text-align:left">Paga en efectivo</span>
			<i>
			    <img src="http://urglo.com/eloboost/miembros/images/efectivo.png" width="40px" align="middle"/>
			    <input type="radio" value="efectivo" name="payt" style="box-shadow: none;"/>
			</i>
		</label>
		';
	}
	...
	$html = str_replace('{chi_t}', $chi_t, $html);

	$html = str_replace('{id_coach}', $id_coach[0]['id_usuario'], $html);
