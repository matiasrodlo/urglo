<?php
	$html = file_get_contents('vistas/admin/lista_trabajos_coachs_cancelados.html');
	...
	$lista_pedidos = $pedidos->lista_trabajos_coach('3');
	...
	for($i=0;$i<count($lista_pedidos);$i++)
	{
		...
		<tr style="background-color: #f2f2f2; color: #000;border-bottom: 1px solid #ddd;">
			...
			<td width="100px">Cancelado</td>
			<td width="100px"><a class="btn_ag" href="http://urglo.com/eloboost/miembros/coachs/trabajos_cancelados/'.$lista_pedidos[$i]['id_trabajo_coach'].'">VER</a></td>
		</tr>';
	}
	if($lista_pedidos == false)
	{
		$html = str_replace('{lista_pedidos}', '<tr style="background-color: #f2f2f2; color: #000;border-bottom: 1px solid #ddd;"><td></td><td></td><td>No hay pedidos cancelados.</td><td></td><td></td><td></td><td></td></tr>', $html);
	}
	else
	{
