<?php
$html = file_get_contents('vistas/admin/lista_pedidos_cancelados.html');
...
if($mis_ordenes == false)
{
	$html = str_replace('{lista_ordenes_boosting}', '
		<tr style="background-color:#fff;color:#000;">
			<td colspan="6" style="border:0px;">No hay ningún pedido cancelado.</td>
		</tr>', $html);
}
else
{
	...
	for($i=0;$i<count($mis_ordenes);$i++)
	{
		...
		$orden = $pedidos->orden_tipo_estado($tipo, $id_pedido, '3');
		if($orden == '')
		{
			$lista_orden = '';
		}
		...
		{
			if($orden[0]['id_elobooster'] == '0' or $orden[0]['id_elobooster'] == null)
			{
				$eloboost = '<h4 style="color:#a00;">NO ASIGNADO</h4>';
			}
			else
			{
				$elo = $usuarios->datos_usuario($orden[0]['id_elobooster']);
				$eloboost = $elo[0]['usuario'];
			}
			if($orden[0]['estado'] == '3')
			{
			...
			if($tipo == '1')
			{
				...
				<tr style="background-color:#fff;color:#000;">
					...
					<td width="100px" style="border:1px solid #dddddd;">'.$eloboost.'</td>
					...
					<td width="67px" style="border:1px solid #dddddd;">
						<a href="http://urglo.com/eloboost/miembros/elobooster/pedidos_cancelados/'.$orden[0]['id_pedido'].'">
