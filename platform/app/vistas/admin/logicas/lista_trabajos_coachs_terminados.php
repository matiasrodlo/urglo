<?php
	$html = file_get_contents(__DIR__ . '/../lista_trabajos_coachs_terminados.html');
	...
	for($i=0;$i<count($lista_pedidos);$i++)
	{
		...
		<tr style="background-color: #f2f2f2; color: #000;border-bottom: 1px solid #ddd;">
			...
			<td width="100px"><a class="btn_ag" href="'.RUTA_INDEX.'coachs/trabajos_terminados/'.$lista_pedidos[$i]['id_trabajo_coach'].'">VER</a></td>
		</tr>';
	}
	
	if($lista_pedidos_libres == '' || count($lista_pedidos) == 0)
	{
		$html = str_replace('{lista_pedidos}', '<tr style="background-color: #f2f2f2; color: #000;border-bottom: 1px solid #ddd;"><td colspan="7" style="text-align:center;padding:20px;">No hay pedidos terminados.</td></tr>', $html);
	}
	else
	{
