<?php
	$html = file_get_contents(__DIR__ . '/../lista_trabajos.html');
	...
	$lista_pedidos = $trabajos->lista_mis_trabajos($id_usuario);
	...
	if(count($lista_pedidos) == 0)
	{
		$html = str_replace('{lista_ordenes_libres}', '
			<tr>
				<td colspan="5">No hay órdenes contratadas.</td>
			</tr>', $html);
	}
	else
	{
		for($i=0;$i<count($lista_pedidos);$i++)
		{
			...
			if (!isset($coach[0]) || !isset($datos_coach[0]) || !isset($cliente[0])) {
				continue;
			}
			//print_r($coach);
			...
			<tr>
				<td>
					<h5 style="font-size:14px;font-weight: normal;line-height: normal;margin:0 0 8px 0;">'.$lista_pedidos[$i]['tipo'].'</h5>
					<p class="price-ord" style="margin:0 0 8px 0;">
						<span class="subtitle">Precio:</span> '.$ganancia_coach.' <span class="subtitle">USD</span>
					</p>
					<p class="subtitle" style="margin:0;font-size:12px;color:#999;">Creado el '.$lista_pedidos[$i]['fecha'].'</p>
				</td>
				<td>
					<h5 style="font-size:14px;font-weight: 500;color:#71B8EE;margin:0;">'.$cliente[0]['usuario'].'</h5>
				</td>
				<td class="center">
					<h5 style="font-size:16px;font-weight: normal;line-height: normal;margin:0;">'.$lista_pedidos[$i]['horas_contratadas'].'</h5>
					<small style="font-size:12px;color:#666;">horas</small>
				</td>
				<td class="center">
					'.$estado.'
				</td>
				<td class="center">
					<a href="'.RUTA_INDEX.'trabajos/'.$lista_pedidos[$i]['id_trabajo_coach'].'">
