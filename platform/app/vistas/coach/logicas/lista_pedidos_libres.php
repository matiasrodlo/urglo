<?php
$html = file_get_contents(__DIR__ . '/../lista_pedidos_libres.html');
$header_desconectado = file_get_contents(__DIR__ . '/../plantilla_coach/header.html');
$footer_desconectado = file_get_contents(__DIR__ . '/../plantilla_coach/footer.html');

$html = str_replace('{footer}', $footer_desconectado, $html);
$html = str_replace('{header}', $header_desconectado, $html);

require_once(__DIR__ . '/../../../funciones/funciones.php');
require_once(__DIR__ . '/../../../modelos/bd.php');
require_once(__DIR__ . '/../../../modelos/usuarios.php');
require_once(__DIR__ . '/../../../modelos/pedidos.php');
require_once(__DIR__ . '/../../../modelos/mensajes.php');
$usuarios = new Usuarios();
$pedidos = new Pedidos();
$mensajes = new Mensajes();

require_once(__DIR__ . '/../../datos_usuario.php');
$mis_ordenes = $pedidos->ordenes();

if($mis_ordenes == false)
{
	$mis_ordenes = array();
}

if(count($mis_ordenes) == 0)
{
	$html = str_replace('{lista_ordenes_coaching}', '
		<tr class="empty-state">
			<td colspan="6" style="border:0px;text-align:center;padding:32px;color:#999;">No hay pedidos libres disponibles.</td>
		</tr>', $html);
}
else
{
	$lista_ordenes_coaching = '';
	for($i=0;$i<count($mis_ordenes);$i++)
	{
		$tipo = $mis_ordenes[$i]['tipo_pedido'];
		$id_pedido = $mis_ordenes[$i]['id_pedido'];
		$orden = $pedidos->orden_tipo_estado($tipo, $id_pedido, '1');
		if($orden == false || empty($orden))
		{
			$lista_orden = '';
		}
		else
		{
			if($orden[0]['id_coach'] == '0' or $orden[0]['id_coach'] == null)
			{
				$coaching = '<span style="color:#a00;font-size:13px;">No asignado</span>';
			}
			else
			{
				$coach_data = $usuarios->datos_usuario($orden[0]['id_coach']);
				$coaching = '<span style="font-size:13px;">'.$coach_data[0]['usuario'].'</span>';
			}
			if($orden[0]['estado'] == '1')
			{
				$estado = '<span style="color:#a00;font-size:13px;">Orden No Pagada</span>';
			}
			if($tipo == '1')
			{
				$lista_orden = '
				<tr>
					<td>
						<h5 style="font-size:14px;font-weight: normal;line-height: normal;margin:0 0 8px 0;">'.$orden[0]['nombre'].'</h5>
						<p class="price-ord" style="margin:0 0 8px 0;"><span class="subtitle">Precio:</span> '.$orden[0]['precio'].' <span class="subtitle">USD</span></p>
						<p class="subtitle" style="margin:0;">Creado el '.$orden[0]['fecha'].'</p>
					</td>
					<td class="center">'.$coaching.'</td>
					<td class="center">'.$estado.'</td>
					<td class="center">
						<a href="'.RUTA_INDEX.'coach/pedidos_libres/'.$orden[0]['id_pedido'].'">
							<div class="btn_details">Ver Detalles</div>
						</a>
					</td>
				</tr>';
			}
			$lista_ordenes_coaching.=''.$lista_orden.'';
		}
	}
	if($lista_ordenes_coaching == '')
	{
		$html = str_replace('{lista_ordenes_coaching}', '
		<tr class="empty-state">
			<td colspan="6" style="border:0px;text-align:center;padding:32px;color:#999;">No hay pedidos libres disponibles.</td>
		</tr>', $html);
	}
	else
	{
		$html = str_replace('{lista_ordenes_coaching}', $lista_ordenes_coaching, $html);
	}
}

	$html = str_replace('{ruta}', RUTA, $html);
	$html = str_replace('{ruta_index}', RUTA_INDEX, $html);
	$html = str_replace('{time}', time(), $html);
echo $html;
?>

